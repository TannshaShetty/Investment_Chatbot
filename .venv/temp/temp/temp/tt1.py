...
# Function to save delimited data to Excel
def save_delimited_data_to_excel(delimited_data, delimiter, file_path):
    try:
        ...
        # Split each row by the delimiter and write to Excel
        for row_idx, row_data in enumerate(rows, start=1):
            row_values = row_data.split(delimiter)  # Split by the delimiter (e.g., ',' or ';')

            # Write the row values into Excel sheet
...
# Function to extract data from Gemini output and convert to delimited format
def convert_to_delimited_format(gemini_output):
    # Assuming the Gemini output is something like:
    # total_amount: 1000, base_amount: 800, tax_amount: 200, recipient_name: "John Doe"
    data_lines = gemini_output.strip().split(",")  # Split by commas to get each key-value pair
    delimited_data = []

    for line in data_lines:
        # Split each line by the colon (:) to get the key and value
        key, value = line.split(":", 1)
        # Clean up any extra spaces around key and value
        delimited_data.append(f"{key.strip()}:{value.strip()}")

    return "\n".join(delimited_data)


# Streamlit UI to handle PDF file and generate Excel
def app():
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            # Gemini output might give us key-value pairs
            delimited_data = gemini_output(img, system_prompt, user_prompt_format)
            st.write("Delimited Invoice Data:")
            st.text(delimited_data)

            # Convert the text response from Gemini to a more structured format
            structured_delimited_data = convert_to_delimited_format(delimited_data)
            st.write("Structured Delimited Data:")
            st.text(structured_delimited_data)
            ...
            if st.button("Save to Excel"):
                ...
                result_message = save_delimited_data_to_excel(structured_delimited_data, delimiter, output_path)
                st.success(result_message)
                st.download_button("Download Excel", data=open(output_path, "rb").read(),
                                   file_name=f"{uploaded_file.name}_invoice.xlsx")
